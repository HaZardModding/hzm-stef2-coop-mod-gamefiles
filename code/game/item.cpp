//-----------------------------------------------------------------------------
//
//  $Logfile:: /red_code/game/item.cpp                                       $
// $Revision:: 10                                                             $
//   $Author:: Ericf                                                          $
//     $Date:: 7/02/01 2:59p                                                  $
//
...
//
// $Log:: /red_code/game/item.cpp                                            $
// 
// 10    7/02/01 2:59p Ericf
// Added support for worldmodel weapons
// 
// 9     5/24/01 10:27a Ericf
// Don't drop to the floor when there's no gravity
// 
// 8     4/30/01 2:47p Ericf
// Ladders and AG47 View Code
// 
// 7     3/28/01 3:25p Steven
// Added warning printfs.
// 
// 6     1/23/01 8:54a Steven
// Added level vars and game vars back in.
// 
// 5     1/16/01 2:11p Steven
// Script C merge.
// 
...
Event EV_Item_SetWeaponModel
	(
	"weaponviewmodel",
	EV_DEFAULT,
   "s",
   "viewmodel_name",
   "Sets the viewmodel tiki name."
	);
...
CLASS_DECLARATION( Trigger, Item, NULL )
	{
      ...
      { &EV_SetAngle,               SetAngleEvent },
		{ &EV_Item_SetWeaponModel,    SetWeaponModel },
		{ NULL, NULL }
	};
...
Item * Item::ItemPickup
   ...
   if ( add_to_inventory )
      {
		if ( weaponmodel != "" )
			item = sent->giveItem( weaponmodel, 0 );
		else
...
void Item::SetWeaponModel
	(
	Event *ev
	)

	{
   weaponmodel = ev->GetString( 1 );
	}
