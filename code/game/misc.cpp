//-----------------------------------------------------------------------------
//
//  $Logfile:: /red_code/game/misc.cpp                                       $
// $Revision:: 10                                                             $
//   $Author:: Ericf                                                          $
//     $Date:: 5/11/01 2:34p                                                  $
//
...
//
// $Log:: /red_code/game/misc.cpp                                            $
// 
// 10    5/11/01 2:34p Ericf
// More Hitching post code
// 
// 9     5/11/01 11:09a Ericf
// Hitching Post Code
// 
// 8     3/28/01 3:25p Steven
// Added warning printfs.
// 
// 7     3/20/01 12:49p Steven
// Fixed a place where TossObjects's didn't setup animate correctly.
// 
// 5     12/11/00 3:17p Steven
// Made str passing const references.
// 
// 4     12/11/00 9:33a Steven
// Changed passing of most vectors to const reference.
// 
// 3     11/29/00 12:56p Steven
// Made mover a pluggable module.
//
// 101   9/26/00 6:58p Aldie
...
/*****************************************************************************/
/*QUAKED trigger_hitchingpost (0.5 0.5 0.5) ? HITCH_ONCE

Entering this trigger will cause the game to check for USE to get on/off AG47

"hitchthread"		The thread that is run when the player hits use in the trigger

If HITCH_ONCE is set, you can only get off the horse, but never get back on
******************************************************************************/

Event EV_HitchingPost_SetThread
	(
	"hitchthread",
   EV_HIDE,
   "s",
...
CLASS_DECLARATION( Trigger, HitchingPost, "trigger_hitchingpost" )
	{
	   { &EV_Touch,							Touch },
		{ &EV_HitchingPost_SetThread,		SetThread },
		{ NULL, NULL }
	};

void HitchingPost::SetThread
	(
	Event *ev
	)
	{
	hitch_thread = ev->GetString( 1 );
	}

void HitchingPost::Touch
	(
	Event *ev
	)
	{
	...
	if ( is_player )
		...
		if ( ent )
			{
			if ( spawnflags & 1 )
				player->SetAtHitchingPost(qtrue, ent->angles, ent->origin, qtrue);
			else
				player->SetAtHitchingPost(qtrue, ent->angles, ent->origin, qfalse);
			}
		}
	}
