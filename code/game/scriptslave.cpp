//-----------------------------------------------------------------------------
//
//  $Logfile:: /Code/DLLs/game/scriptslave.cpp                               $
// $Revision:: 57                                                             $
//   $Author:: Steven                                                         $
//     $Date:: 11/06/02 2:24p                                                 $
//
...
CLASS_DECLARATION( Trigger, ScriptSlave, "script_object" )
{
	...
	{ &EV_ScriptSlave_MoveToPosition,			MoveToPositionEvent },
	...
	{ &EV_ScriptSlave_StopEvent,				StopEvent },
	{ &EV_ScriptSlave_ContinueEvent,			ContinueEvent },
	{ &EV_SetGameplayDamage,					setDamage },
	{ &EV_ScriptSlave_HandlesDamage,			HandlesDamage },
	{ &EV_ScriptSlave_DamageEffect,				setDamageEffect },

	//1ST PLAYABLE HACK STUFF
	{ &EV_ScriptSlave_Hack_SetTriggerParms,     Hack_AddParms             },
	{ &EV_ScriptSlave_Hack_GetForceFieldNumber, Hack_GetForceFieldNumber  },
	{ &EV_ScriptSlave_Hack_GetTriggerNumber,    Hack_GetTriggerNumber     },
	{ &EV_ScriptSlave_Hack_GetScanner,          Hack_GetScanner           },
...
void ScriptSlave::UseFunc( Event *ev )
{
	...
	else if ( key.length() )
	{
		if ( !other->isSubclassOf( Sentient ) || !( ( (Sentient *)other )->HasItem( key.c_str() ) ) )
		{
			...
			gi.centerprintf ( other->edict, "$$ItemNeeded$$%s", item->getName().c_str() );
			delete item;
			return;
		}
		else if ( other->isSubclassOf( Sentient ) )
		{
			...
			{
				gi.centerprintf ( other->edict, "$$NeedToUse$$ %s", key.c_str() );
...
CLASS_DECLARATION( ScriptSlave, ScriptModel, "script_model" )
{
	...
	{ &EV_ScriptModel_SetAnimDriven,	SetAnimDrivenEvent },
...
{
	{ &EV_ScriptSkyOrigin_SetBasePosition,					SetBasePosition },
	{ &EV_ScriptSkyOrigin_SetPlayerBasePosition,			SetPlayerBasePosition },
	{ &EV_ScriptSkyOrigin_SetTranslationMultiplier,			SetTranslationMultiplier },
	{ &EV_ScriptSkyOrigin_SetMaxDistance,					SetMaxDistance },
