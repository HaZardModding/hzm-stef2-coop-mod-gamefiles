...
//
// $Log:: /EF2/game/sentient.cpp                                             $
// 
// 34    8/29/01 6:05p Steven
// Fixed a this pointer issue with the Intel compiler.
// 
// 33    6/05/01 12:09p Sketcher
// Modified Gibbing stuff slightly -- Needs a more thorough reworking
// 
// 32    6/05/01 7:41a Sketcher
// Added a new gib
// 
// 31    5/31/01 9:27a Steven
// Added regional damage stuff and made blood spurt form the centroid of the
// bone hit.
// 
// 30    5/21/01 10:34a Ericf
// Fixed a ghost shirt damage bug
// 
// 29    5/01/01 1:55p Steven
// Added bone damage stuff.
// 
// 28    5/01/01 11:19a Sketcher
// Added an ActorFlag for swapping damage skins
// 
// 27    4/18/01 1:37p Ericf
// Alternate Draw Bow Strain event
// 
// 26    3/28/01 3:25p Steven
// Added warning printfs.
// 
// 25    3/28/01 11:28a Ericf
// Added Switch Mode weapons code
// 
// 24    3/27/01 4:31p Sketcher
// Added a rotatetoenemy behavior
// 
// 23    26.03.01 11:44a Ericf
...
CLASS_DECLARATION( Entity, Sentient, NULL )
   {
      ...
      { &EV_Sentient_StartChargeFire,		StartChargeFire },
      ...
      { &EV_Damage,                       ArmorDamage },
		...
		{ &EV_Sentient_AddResistance,			AddResistance },
		{ &EV_Sentient_RemoveResistance,	   RemoveResistance	},
...
void Sentient::StartChargeFire
   ...
   firing_frame = ev->GetAnimationFrame();*/
	...
	if ( ( activeWeapon ) && activeWeapon->ReadyToFire( mode ) )
      ...
      if ( mode == FIRE_MODE1 )
		   activeWeapon->animate->RandomAnimate( "charge", EV_Weapon_DrawBowStrain);			
      else if ( mode == FIRE_MODE2 )
         activeWeapon->animate->RandomAnimate( "alternatecharge", EV_Weapon_AltDrawBowStrain );
		}
   }
   ...
   activeWeapon = activeWeaponList[ number ];
	...
	if ( activeWeapon )
		...
		if ( activeWeapon->animate->HasAnim("fire_stop") )
			activeWeapon->animate->RandomAnimate("fire_stop");
		else
...
void Sentient::ActivateWeapon
   ...
   weapon->animate->RandomAnimate( "raise" );
   }
...
void Sentient::ArmorDamage
   ...
   meansofdeath   = ev->GetInteger( 9 );
	...
	if ( ev->NumArgs() > 9 )
		surface_number = ev->GetInteger( 10 );

	if ( ev->NumArgs() > 10 )
		bone_number = ev->GetInteger( 11 );
   ...
   if ( deadflag )
      {
		...
		if ( ShouldBleed( meansofdeath, true ) )
			{
			AddBloodSpurt( direction, bone_number );
...
//		float water_level;
      ...
      player_angle = player->GetTorsoAngles();
		...
		//Standard Armor -- If enough armor remains then the armor will absorb 80% of the damage,
		//If not enough armor remains then the armor absorbs as follows Armor/Damage and Armor is then
		//Set to 0.
		if ( meansofdeath != MOD_LIFEDRAIN )
			{
			...
			if (currentArmor)
				...
				if ( currentArmor > damage )
					{
					player->AdjustArmorValue( -damage );
					damage *= .8;				
					}
				else
					{
					damage_absorbed = currentArmor / damage;
	...
	/*	if ( meansofdeath != MOD_LIFEDRAIN )
		...
		// Ghost Shirt Armor Stuff
		if ( meansofdeath != MOD_LIFEDRAIN )
			{
			float GSValue = player->GetGhostShirt();

			damage_absorbed =  ( GSValue * .5 ) / 100;
			damage *= (1-damage_absorbed);
			}
      }
   ...
   if ( g_debugdamage->integer )
	...
	if ( ( damage > 0 ) && ( surface_number != -1 ) )
      {
		if ( this->isSubclassOf( Actor ) )
			...
			if( theActor->GetActorFlag(ACTOR_FLAG_USE_DAMAGESKINS) )
				{
				if ( !edict->s.surfaces[ surface_number ] )
				   edict->s.surfaces[ surface_number ]++;
	...
	SetRegionalDamage( surface_number, bone_number, direction );
...
void Sentient::AddBloodSpurt
	...
	// Default the blood spurt to the sentient's centroid
	...
	if ( bone_number >= 0 )
		...
		if ( parent_bone_number >= 0 )
			...
			bone_centroid = (bone_pos + parent_bone_pos ) * 0.5;
...
str Sentient::GetGibName
	...
	if ( blood_model == "fx_bspurt.tik" )
		...
		gib_name = "fx_ggib";
	else if ( blood_model == "fx_bspurt_green.tik" )
		gib_name = "fx_ggib";
...
void Sentient::Archive
   ...
   arc.ArchiveInteger( (int *)&poweruptype );
...
void Sentient::AddResistance
	...
	if(new_resistance)
		{
		new_resistance->MODIndex = MOD_string_to_int(ev->GetString(1));
...
void Sentient::RemoveImmunity
...

   {
	...
	number_of_immunities = ev->NumArgs();
		...
		for ( int x = 1; x <= resistances.NumObjects(); x++ )
			{
				old_resistance = resistances.ObjectAt(i);
				if( old_resistance->MODIndex == old_immunity )
					{
						resistances.RemoveObject(old_resistance);
...
void Sentient::RemoveResistance
	(
	Event *ev
	)
	...
	resistance_idx = MOD_string_to_int( ev->GetString(1) );
...
int Sentient::GetResistanceModifier				  
	(
	int meansofdeath
	)
