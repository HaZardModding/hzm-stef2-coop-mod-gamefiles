//-----------------------------------------------------------------------------
//
//  $Logfile:: /red_code/game/steering.cpp                                   $
// $Revision:: 9                                                              $
//   $Author:: Sketcher                                                       $
//     $Date:: 7/02/01 2:37p                                                  $
//
...
//
// $Log:: /red_code/game/steering.cpp                                        $
// 
// 9     7/02/01 2:37p Sketcher
// Modified JumpTo to take in a vertical speed
// 
// 8     4/23/01 10:07a Steven
// Fixed a warning.
// 
// 7     4/20/01 11:35a Steven
// Had to take out goal optimization because it was causing problems.
// 
// 6     4/16/01 11:30a Steven
// Improved pathfinding.
// 
// 5     3/28/01 3:25p Steven
...
qboolean	Seek::Evaluate
   ...
   delta.z = 0;
	...
	// Throttle movement a little if near predicted position

	// Calculate estimated number of frames to get to position

	delta = predictedposition - origin;
	dist = delta.length();
	frames = dist / self.total_delta.length();

	// Calculate estimated number of frames to turn towards position

	turn_frames = ( steeringforce.y / self.turnspeed );

	if ( turn_frames < 0 )
		turn_frames = -turn_frames;

	// If not going to be able turn in time slow down a little

	if ( turn_frames > frames && turn_frames > 0 )
		self.total_delta *= frames / turn_frames;
...
Path *FollowPath::SetPath
   ...
   goal = PathManager.NearestNode( to, &self );
	...
	// Make sure we aren't calculating the same path as last time

	/* if ( path )
		{
		last_goal = path->GetNode( path->NumNodes() );
	...
	if ( ai_advanced_pathfinding->integer )
		{
		PathNode *start_node;
		PathNode *last_node;
		PathNode *end_node;
		...
		if ( !path || path->NumNodes() < 2 )
			return path;

		new_path = new Path();

		start_node = path->GetNode( 1 );
		end_node   = path->GetNode( path->NumNodes() );
		last_node  = start_node;

		// Always add the first path node

		new_path->AddNode( start_node );

		// Determine which path nodes to add to optimized list

		for( i = 2 ; i <= path->NumNodes() ; i++ )
			{
			current_node = path->GetNode( i );

			if ( !self.CanWalkToFrom( start_node->origin, current_node->origin, 5.0 ) )
				{
				// Can't walk to this node directly so add the last one to list

				if ( last_node != start_node )
					{
					new_path->AddNode( last_node );
					start_node = last_node;
					}
				}
			else
				{
				if ( self.CanWalkToFrom( current_node->origin, end_node->origin, 5.0 ) )
					{
					// Can walk directly from this node to the end so add this node and we are done

					new_path->AddNode( current_node );
					break;
					}
				}
		...
		// Always add the last path node

		new_path->AddNode( end_node );

		// Replace the first path with the optimized path
...
qboolean	FollowPath::Evaluate
   ...
   targetpos = currentNode->origin;
	...
	// Modify the target postion a little to get back on the path
	...
	if ( ai_advanced_pathfinding->integer )
		...
		if ( lastnode )
			...
			node_dir = currentNode->origin - lastnode->origin;
			dist = node_dir.normalize();

			pos_dir = self.origin - lastnode->origin;

			dist_along_path = node_dir * pos_dir;

			// Make sure we aren't too close to the target position

			delta = targetpos - self.origin;

			frame_dist = self.animate->frame_delta.length();

			if ( delta * delta > frame_dist )
				{
				// Make sure we are between the 2 path nodes

				if ( ( dist_along_path > 0 ) && ( dist_along_path < dist ) )
					{
					// Calculate the position on the path

					path_pos = lastnode->origin + node_dir * dist_along_path;

					// Calculate the vector/dist to the point on the path

					path_dir = path_pos - self.origin;
					dist_to_path = path_dir.length();

					// If not on the path move closer

					if ( dist_to_path > 0.5 )
						{
						// Move target position to move towards path some depending on how far away we are

						if ( dist_to_path > frame_dist / 4 )
							{
							float path_percent;

							// Calculate how much to move towards path

							path_dir = path_pos - self.origin;
							path_dir.normalize();

							path_percent = ( dist_to_path / frame_dist ) / 4;
							...
							path_dir *= path_percent;
							delta.normalize();
							delta += path_dir;
							delta.normalize();

							targetpos = self.origin + delta * frame_dist * 5;
							}
						else
							{
							// Calculate exactly how much to move towards path to get right on it

							dist_along_path = sqrt( frame_dist * frame_dist - dist_to_path * dist_to_path );

							path_pos = path_pos + node_dir * dist_along_path;
							path_dir = path_pos - self.origin;
							dist_to_path = path_dir.normalize();

							// Change target position

							targetpos = self.origin + path_dir * frame_dist * 5;
							}
						}
					}
				}
   ...
   if ( delta * delta <= self.animate->frame_delta * self.animate->frame_delta )
      {
   ...
   if ( mode != CHASE_MODE_WANDER )
	...
	if ( mode == CHASE_MODE_DIRECT )
		...
		if ( nextwalktotime < level.time )
			{
			if ( !self.CanWalkTo( target, 0.0, target_entnum ) )
				{
				mode = CHASE_MODE_USE_PATH;
				return true;
				}
		...
		if ( nextpathtime < level.time )
			{
			if ( nextwalktotime < level.time && self.CanWalkTo( target, 0.0, target_entnum ) )
				{
				mode = CHASE_MODE_DIRECT;
				nextwalktotime = level.time + newpathrate;
				return true;
				...
				if ( !path )
					...
					trace_end_pos = target - Vector( "0 0 1000" );

					trace = G_Trace( target, vec_zero, vec_zero, trace_end_pos, NULL, MASK_SOLID, false, "Chase" );

					goal_node = self.NearestNodeInPVS( Vector( trace.endpos ) );
					...
					if ( !path )
						...
						if ( allow_fail )
							{
							gi.WDPrintf( "No path found for actor %d", self.entnum );

							if ( goalnode )
								gi.WDPrintf( " , target node %s\n", goalnode->targetname.c_str() );
							else if ( goalent )
								gi.WDPrintf( " , target ent %d\n", goalent->entnum );
							else
								gi.WDPrintf( " , target pos (%f,%f,%f)\n", goal.x, goal.y, goal.z );
		...
		if ( path )
			...
			if ( !follow.Evaluate( self ) )
				...
				if ( goalnode )
					{
					self.animate->frame_delta = goalnode->origin - self.origin;
					return false;
					}
				}
...
void Jump::Begin
   ...
   traveltime = self.JumpTo( goal, speed, vertical_speed );
...
qboolean	Jump::Evaluate
   ...
   switch( state )
      ...
      case 0:
         ...
         break;
		case 1:
			if ( animdone && self.animate->HasAnim( "fall" ) )
            {
            animdone = false;
