//-----------------------------------------------------------------------------
//
//  $Logfile:: /red_code/game/weaputils.cpp                                  $
// $Revision:: 29                                                             $
//   $Author:: Ericf                                                          $
//     $Date:: 7/02/01 2:24p                                                  $
//
...
//
// $Log:: /red_code/game/weaputils.cpp                                       $
// 
// 29    7/02/01 2:24p Ericf
// Fixed the randomness on the "drunk" event
// 
// 28    6/25/01 5:26p Ericf
// Grenade Launcher code
// 
// 27    5/31/01 9:28a Steven
// Made projectiles do full trace so that they would cause area damage.
// 
// 26    5/30/01 6:02p Sketcher
// Added a flag on the melee damage notifying actor that the melee attack hit
// the world
// 
// 25    5/29/01 8:18a Sketcher
// Working on allowing actors to respond defensively to hitscan weapons...
// Ahhh yeah!!!
// 
// 24    5/23/01 1:10p Ericf
// Added a default radius_damage
// 
// 23    5/03/01 3:48p Ericf
// 
// 22    5/03/01 2:22p Ericf
// Removed debug weapon decals
// 
// 21    5/01/01 3:51p Ericf
// Fixed camera snapping after the rocket explodes
// 
// 20    5/01/01 1:56p Steven
// Added bone damage stuff.
// 
// 19    4/30/01 2:46p Ericf
// Removed TOW cam
// 
// 18    4/20/01 1:20p Sketcher
// projectiles can now "stick" into walls
// 
// 17    3/28/01 3:25p Steven
// Added warning printfs.
// 
// 16    15.03.01 2:51p Ericf
// Fixed small bug with medicine bow
// 
// 15    15.03.01 12:16p Ericf
// Fixed some bugs in redemptive damage system
// 
// 14    15.03.01 11:42a Ericf
// 
// 13    15.03.01 11:36a Ericf
// Added Redemptive Damage
// 
// 6     6.02.01 4:37p Ericf
// Debug bloodsplat code (commented out)
// 
// 5     1/30/01 2:15p Steven
// Area based damage stuff.
// 
// 4     12/11/00 3:17p Steven
// Made str passing const references.
// 
// 3     12/11/00 9:34a Steven
// Changed passing of most vectors to const reference.
//
// 126   9/28/00 1:46p Aldie
// Yet another multiplayer update
//
// 125   9/22/00 4:01p Aldie
// Removed comment
//
...
Event EV_Projectile_RedemptiveDamage
   (
   "redemptivedamage",
   EV_DEFAULT,
   NULL,
   NULL,
   "Sets the redemptive damage of the projectile"
   );
...
CLASS_DECLARATION( Entity, Projectile, NULL )
	{
      ...
      { &EV_Killed,		      	               Explode },
		{ &EV_Stop,			      	               Stopped },
		{ &EV_Projectile_SetOwnerControl,			SetOwnerControl },
		{ &EV_Projectile_RedemptiveDamage,			SetRedemptiveDamage },
		{ &EV_Projectile_StickOnTouch,            StickOnTouch },
		{ &EV_Projectile_TriggerDetonate,         TriggerDetonate },
		{ NULL, NULL }
	};
...
void Projectile::TriggerDetonate
   (
   Event *ev
   )
   ...
   triggerdetonate = ev->GetBoolean( 1 );
	if ( triggerdetonate )
		PostEvent( EV_Projectile_Prethink, 0.05 );
   }

void Projectile::SetOwnerControl
   (
   Event *ev
   )
...
// This is really a Think function now instead of a "Prethink"
void Projectile::Prethink
   ...
   if ( drunk )
   	angles += Vector( G_CRandom( 1 ), G_CRandom( 1 ), 0 );

	if ( this->owner == 0 )
		{
		own = G_GetEntity(owner);
		if ( own->isSubclassOf( Player ) )
			{
			player = (Player *)own;

			if ( ownercontrol )
				{
				if ( firstTimeOwnerControl )
					{
...
void Projectile::SetRedemptiveDamage
   (
   Event *ev
   )

   {
   redemptivedamage = ev->GetFloat( 1 );
   }
...
void Projectile::Touch
   ...
   // Remove the projectile
	if(!stick_on_touch)
	   ...
	   end_pos = forward * 1000 + origin;
	   trace = G_Trace( origin, vec_zero, vec_zero, end_pos, owner, MASK_PROJECTILE, false, "ProjectileAttack" );
...
void ExplosionAttack
   ...
   if ( explosionModel.length() )
      ...
      RadiusDamage( explosion,
                    owner,
                    explosion->radius_damage * scale * damageMult,
                    ignore,
