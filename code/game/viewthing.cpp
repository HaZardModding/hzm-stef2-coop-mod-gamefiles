//-----------------------------------------------------------------------------
//
//  $Logfile:: /red_code/game/viewthing.cpp                                 $
// $Revision:: 29                                                             $
//   $Author:: Steven                                                         $
//     $Date:: 6/12/01 9:00a                                                  $
//
...
//
// $Log:: /red_code/game/viewthing.cpp                                      $
// 
// 29    6/12/01 9:00a Steven
// Made viewthings default to an idle animation if it has one.
// 
// 28    5/30/01 10:18a Steven
// Added viewoffset.
// 
// 27    8.05.01 12:24p Kharward
// Modified ::Shoot to better enable shooting thru other objects bounding
// boxes.
// 
// 26    4.05.01 4:01p Kharward
// Fixed a bug in the bell curve randomization.
// 
// 25    4.05.01 3:29p Kharward
// Added cvars to control randomization of viewspawns.
// 
// 24    3.05.01 3:02p Kharward
// Added support for randomizing scale and rotation of newly spawned objects.
// 
// 23    3.05.01 2:03p Kharward
// Added support for tracking number of viewthings spawned.  Also added lots
// of comments.
// 
// 22    2.05.01 7:43p Kharward
// Switched the usebox cvar to be true (do use bounding boxes when
// calculating traces for view items shot out with the mouse)
// 
// 21    2.05.01 7:40p Kharward
// Added more view commands for objplacer menu.
// 
// 20    1.05.01 6:03p Kharward
// Changed Save functionality so that angles go out in hopefully correct
// order.
// 
// 19    27.04.01 5:04p Kharward
// Added the code once again for rotating objects when shot with viewshoot.
// Also using current object's bounding box for offsets
// 
// 18    24.04.01 7:21p Kharward
// Added the viewshoot command.  You can now use the objplacer.cfg to place
// and manipulate objects through wtih the mouse.
// 
// 17    4/23/01 4:33p Ericf
// More view edit work
// 
// 16    23.04.01 4:03p Kharward
// Fixed a bug where changing an object in the middle of a set of objects
// would cause an object to be inadvertently deleted.
// 
// 15    23.04.01 3:24p Kharward
// Fixed a problem where only one translation could be applied to a viewthing
// at a time.
// 
// 14    23.04.01 2:33p Kharward
// Dropped items now drop up to 10000 feet if drop mode is on.
// 
// 13    20.04.01 7:18p Kharward
...
CLASS_DECLARATION( Listener, ViewMaster, NULL )
	{
		...
		{ &EV_ViewThing_SetStatic,          PassEvent },
		{ &EV_ViewThing_SetOrigin,			PassEvent },
		{ &EV_ViewThing_SetAnim,			PassEvent },
		{ &EV_ViewThing_NextMorph,			PassEvent },
		{ &EV_ViewThing_PrevMorph,			PassEvent },
		{ &EV_ViewThing_Morph,				PassEvent },
		{ &EV_ViewThing_Unmorph,			PassEvent },
		{ &EV_ViewThing_ChangeRoll,         PassEvent },
		{ &EV_ViewThing_ChangePitch,        PassEvent },
		{ &EV_ViewThing_ChangeYaw,          PassEvent },
		{ &EV_ViewThing_Flash,              PassEvent },
		{ &EV_ViewSet_Load,                 LoadSet   },
		{ &EV_ViewSet_Save,                 Save      },
		{ &EV_ViewThing_Copy,               Copy      },
		{ &EV_ViewThing_Shoot,              Shoot     },
		{ &EV_ViewThing_Delete,             Delete    },
		{ &EV_ViewThing_TranslateX,         SetXTranslation },
		{ &EV_ViewThing_TranslateY,         SetYTranslation },
		{ &EV_ViewThing_TranslateZ,         SetZTranslation },
		{ &EV_ViewList_PrevInSet,           PrevModelInSet },
		{ &EV_ViewList_NextInSet,           NextModelInSet },
		{ &EV_ViewList_LiftModel,           SetZTranslation },
		{ &EV_ViewList_ToggleDrop,          ToggleDrop },
		{ &EV_ViewList_Jump,                JumpToModel },
		{ &EV_ViewThing_Pull,               PullToCamera },
		{ &EV_ViewThing_ResetOrigin,        ResetOrigin },
		{ &EV_ViewThing_XTrans,             XTranslate },
		{ &EV_ViewThing_YTrans,             YTranslate },
		{ &EV_ViewThing_ZTrans,             ZTranslate },
		{ &EV_ViewThing_Offset,             Offset },
		{ NULL, NULL }
	};
...
void ViewMaster::LoadModelsInSet
	(
	const str &setFileName
	)
	{	
		char  *buffer;
		char  *buf;
		...
		if ( gi.FS_ReadFile( setFileName, ( void ** )&buf, qtrue ) != -1 )
		{
			buffer = buf;
			while ( 1 )
			{
...
void ViewMaster::DisplayCurrentModelInSet
	(
	bool spawnAtLastOrigin
	)
	{
...
void ViewMaster::Shoot
	...
	)
	{
		...
		Vector pos = Vector( trace.endpos ) + ( Vector( trace.plane.normal ) * 0.2 );
...
//              
//================================================================
void ViewMaster::NextModelInSet
	(
	Event *
	)
	{
...
//              
//================================================================
void ViewMaster::PrevModelInSet
	(
	Event *ev
	)
	{
...
void ViewMaster::SetXTranslation
	(
	Event *ev
	)
	{
...
void ViewMaster::SetYTranslation
	(
	Event *ev
	)
	{
...
void ViewMaster::SetZTranslation
	(
	Event *ev
	)
	{
...
//              
//================================================================
void ViewMaster::spawnAtPosition
	(
	const str&    modelName,
	const Vector& pos
	)
...
//              
//================================================================
void ViewMaster::_setToPosition
	(
	Vector& pos
	)
	{
...
//              
//================================================================
void ViewMaster::_randomizeViewthing
	(
	Viewthing* viewthingPtr
	)
	{
		if (!viewthingPtr)
		{
		...
		{
			// Randomly scale
			float die1        = G_Random(100);
			float die2        = G_Random(100);
...
CLASS_DECLARATION( Entity, Viewthing, "viewthing" )
	{
		...
		{ &EV_ViewThing_SetStatic,          SetStatic },
		...
		{ &EV_ViewThing_NextMorph,			NextMorph },
		{ &EV_ViewThing_PrevMorph,			PrevMorph },
		{ &EV_ViewThing_Morph,				Morph },
		{ &EV_ViewThing_Unmorph,			Unmorph },
		{ &EV_ViewThing_ChangeRoll,         ChangeRollEvent },
		{ &EV_ViewThing_ChangePitch,        ChangePitchEvent },
		{ &EV_ViewThing_ChangeYaw,          ChangeYawEvent },
		{ &EV_ViewThing_Flash,              Flash },
		{ NULL, NULL }
	};
...
void Viewthing::ThinkEvent
   ...
   if ( ( animstate > 0 ) && ( Viewmodel.current_viewthing == this ) )
      ...
      if ( f != lastframe )
         ...
         float time;
         lastframe = f;
         time = f * animate->AnimTime() / animate->NumFrames();
...
void Viewthing::ChangeRollEvent
	(
	Event *ev
	)
	{
...
void Viewthing::ChangePitchEvent
	(
	Event *ev
	)
	{
