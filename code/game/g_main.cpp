...
//
// $Log:: /EF2/game/g_main.cpp                                               $
// 
// 17    8/23/01 2:44p Steven
// Removed all rff stuff.
// 
// 16    8/21/01 8:45a Steven
// Changed all references to the game name to use the #define in q_shared.h.
// 
// 15    7/18/01 8:04a Sketcher
// Completed first step in Heuristics System re-vamp.
// 
// 14    5/22/01 3:11p Ericf
// Multiplayer Options UI stuff
// 
// 13    12.05.01 4:59p Kharward
// Added a call to DM_Manager::Init to ensure that the dmManager has a chance
// to initialize.
// 
// 12    5/02/01 2:04p Steven
// Merged in Strange Company stuff.
// 
// 11    4/24/01 3:41p Steven
// Added sv_showsplines.
// 
// 10    4/24/01 7:33a Steven
// Added dialog text.
// 
// 9     3/28/01 3:24p Steven
// Added warning printfs.
// 
// 7     1/24/01 2:08p Sketcher
// Fixed a compiler warning
// 
// 6     1/23/01 10:37a Sketcher
// Added Code to allow for Saving and Loading of Heuristic File
// 
// 5     1/23/01 8:54a Steven
// Added level vars and game vars back in.
// 
// 4     1/16/01 2:11p Steven
// Script C merge.
// 
// 3     12/20/00 2:43p Steven
// Changed FAKK to Redemption in a bunch of places.
//
// 97    10/06/00 4:50p Aldie
// Intermission code
//
// 96    10/02/00 11:54a Aldie
// Fix for voting sv_nextmap
//
// 95    9/28/00 6:51p Aldie
// Added more multiplayer options (voting)
//
// 94    9/26/00 6:58p Aldie
// Another dm update - lots of stuff added
//
// 93    9/22/00 6:10p Aldie
// Skin support
//
// 92    9/21/00 3:51p Aldie
// Update for G_ExitLevel
//
// 91    9/20/00 6:58p Aldie
// Another temporary checkin for deathmatch stuff
//
// 90    8/30/00 4:35p Aldie
...
qboolean G_ArchiveLevel
   ...
   try
      ...
      if ( arc.Loading() )
		...
		if ( arc.Loading() )
         {
			dialogText.ClearEntries();
	      dialogText.LoadEntries( level.mapname.c_str() );
         }
...
/*
      if ( ( level.m_intermission_angle != vec_zero ) && ( level.m_intermission_origin != vec_zero ) )
         {
...
void G_CheckExitRules
   (
   void
   )

   {
   // Check whether or not to exit the level in deathmatch
   if ( !DM_ACTIVE )
      {
      return;
      }

   // We're in an intermission, so check to see if the intermission is over
   if ( level.intermissiontime )
      {
   ...
   // Check time limit done
   if ( timelimit->integer )
      {
      if ( level.time >= timelimit->integer*60 )
         {
...
void G_RunFrame
   ...
   try
      ...
      showentnums = ( sv_showentnums->integer && ( !DM_ACTIVE || sv_cheats->integer ) );
      ...
      // Check for voting conditions in deathmatch
   ...
   try
      ...
      */
      ...
      // set deathmatch model
	   s = Info_ValueForKey( userinfo, "dm_playermodel" );
      if ( !s )
         {
         assert( 0 );
         return;
         }
	   strncpy( ent->client->pers.dm_playermodel, s, sizeof( ent->client->pers.dm_playermodel ) - 1 );
