//-----------------------------------------------------------------------------
//
//  $Logfile:: /red_code/game/program.cpp                                     $
// $Revision:: 3                                                              $
//     $Date:: 6/18/01 11:10a                                                 $
//
...
//
// $Log:: /red_code/game/program.cpp                                          $
// 
// 3     6/18/01 11:10a Sketcher
...
// Script C merge.
// 
// 12    9/14/00 10:25a Sfleming
// added CountUsedStrings() for debugging purposes
// 
// 11    9/13/00 1:38p Pmack
// added explicit {}'s in Program::AllocString
// 
// 10    8/28/00 11:53p Sfleming
// save/load first pass
// 
// 7     5/09/00 12:10p Pmack
// Added hook so scripts can talk to dialogtext.
// 
// 6     4/05/00 3:56p Jimdose
// Fakk2 merge
// 
//
// DESCRIPTION:
...
inline void type_t::Archive( Archiver &arc )	// SMF
{
	int i;
...
inline void dfunction_t::Archive( Archiver &arc )	// SMF
{
	Class::Archive( arc );
	...
	arc.ArchiveRaw( parm_size, sizeof( parm_size[0] ) * MAX_PARMS );
	arc.ArchiveRaw( parm_type, sizeof( parm_type[0] ) * MAX_PARMS );
}
...
inline void Program::Archive( Archiver &arc )
{
	...
	arc.ArchiveRaw( pr_globals, sizeof( pr_globals[0] ) * MAX_REGS );
...
CLASS_DECLARATION( Class, dfunction_t, NULL )		// SMF
{
	{ NULL, NULL }
};

CLASS_DECLARATION( Class, type_t, NULL )		// SMF
{
	{ NULL, NULL }
};

CLASS_DECLARATION( Class, def_t, NULL )		// SMF
{
	{ NULL, NULL }
};
...
void Program::CreateDefForEvent
   (
   Event *ev
   )
...
============
*/
void Program::FreeString( int idx )		// SMF
{
...
============
*/
int Program::CountUsedStrings()			// SMF
{
	int i, count = 0;
...
void Program::BeginCompilation
   ...
   locals_start = numpr_globals;
	...
	strings[0].inuse = true;	// always used as the return string
   ...
   GetDef( &type_void, "cam", NULL, true, NULL );
	...
	//PGM
   GetDef( &type_void, "dialogtext", NULL, true, NULL );
	//pgm
   }
...
==============
*/
bool Program::FinishCompilation
   (
   void
   )
...
void Program::Compile
   ...
   s_file = filename;
   if ( gi.FS_ReadFile( filename, ( void ** )&src, qtrue ) < 0 )
      {
      s_file = oldfile;
      gi.WPrintf( "Couldn't load %s\n", filename );
      throw "Error";
      }
...
TargetList *Program::getTargetList
   (
   int offset
   )
