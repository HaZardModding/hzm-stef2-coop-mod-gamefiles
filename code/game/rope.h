...
class Rope : public ScriptSlave
   ...
   private:
      ...
      Vector   CalculateRiderPosition( const Vector &position );
      bool     CanMove( const Vector &start, const Vector &location );
      void     Move( const Vector &location );
      void     Update( Vector &midpos );

      void     RopeThink( Event * ev );
      void     Touch( Event * ev );
      void     Setup( Event * ev );
      void     SetShader( const str &name );
...
public:
   ...
   void           SetShader( Event *ev );
	...
	void           SetTouchVelocity(const Vector &fullvel, RopePiece *touched_piece, int set_z);
