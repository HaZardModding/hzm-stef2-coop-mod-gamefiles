...
//
// $Log:: /EF2/game/navigate.cpp                                             $
// 
// 15    1/02/02 2:38p Sketcher
// Renamed AI_REST to AI_HIBERNATE
// 
// 14    12/17/01 3:12p Steven
// Fixed a crash in navigate, wasn't defaulting ai_maxnode to -1 like it
// should have.
// 
// 13    11/05/01 2:33p Sketcher
// Changed Flags on PathNodes
// 
// 12    11/02/01 8:04a Sketcher
// Added WORK and REST nodes
// 
// 11    10/08/01 7:52a Steven
// Lint first pass.
// 
// 10    5/02/01 3:10p Steven
// In NearestNode don't consider nodes without any children.
// 
// 9     4/16/01 11:29a Steven
// Improved pathfinding.
// 
// 8     4/13/01 8:41a Steven
// Optimized path nodes.
// 
// 7     3/28/01 3:25p Steven
...
qboolean PathNode::ClearPathTo
   ...
   width = NUM_WIDTH_VALUES * WIDTH_STEP * 0.5f;
...
void PathNode::ConnectTo
	(
	const PathNode *node,
	const byte maxheight[ NUM_WIDTH_VALUES ],
	float cost,
	const Door *door
	)
...
void PathNode::FindChildren
   ...
   //if ( !( contents & MASK_WATER ) )
	...
	PathManager.CancelEventsOfType( EV_AI_OptimizeNodes );
	PathManager.PostEvent( EV_AI_OptimizeNodes, 0.0f );
	}
...
void DrawAllConnections
   ...
   playerorigin[ 2 ] += g_entities[ 0 ].client->ps.viewheight;
	...
	for( node = NodeList; node != NULL; node = node->chain )
      ...
      if ( shownums )
         {
         G_DrawDebugNumber( node->origin + Vector( 0.0f, 0.0f, 14.0f ), node->nodenum, 1.5f, 1.0f, 1.0f, 0.0f );
         }
...
CLASS_DECLARATION( Class, PathSearch, NULL )
	{
		...
		{ &EV_AI_OptimizeNodes,       OptimizeNodes },
...
qboolean PathSearch::CanDropPath( const PathNode *node, const PathNode *node2, const pathway_t *path )
	{
	int i, j, k;
	...
	for( i = 0 ; i < node->numChildren ; i++ )
		{
		inbetween_node = AI_GetNode( node->Child[ i ].node );

		for( j = 0 ; j < inbetween_node->numChildren ; j++ )
			{
			temp_node = AI_GetNode( inbetween_node->Child[ j ].node );
			...
			if ( temp_node == node2 )
				...
				for( k = 0 ; k < NUM_WIDTH_VALUES ; k++ )
					...
					if ( ( path->maxheight[ k ] > node->Child[ i ].maxheight[ k ] ) || 
						  ( path->maxheight[ k ] > inbetween_node->Child[ j ].maxheight[ k ] ) )
						{
						found_better_path = false;
						break;
						}
...
void PathSearch::OptimizeNodes
   ...
   PathNode *node;
	PathNode *node2;
	int drop[ NUM_PATHSPERNODE ];
	int number_children_left;
	int current_child;
	int dropped_paths;
	int kept_paths;
	...
	for( i = 0; i < MAX_PATHNODES; i++ )
		...
		if ( pathnodes[ i ] )
			...
			for( j = 0 ; j < node->numChildren ; j++ )
				{
				node2 = AI_GetNode( node->Child[ j ].node );

				if ( CanDropPath( node, node2, &node->Child[ j ] ) )
					{
					drop[ j ] = true;
					dropped_paths++;
					}
					...
					{
					drop[ j ] = false;
					number_children_left++;
					kept_paths++;
					}
				}

			// Drop all unneeded children
			...
			for( j = 0 ; j < node->numChildren ; j++ )
				...
				if ( !drop[ j ] )
					...
					if ( j != current_child )
						{
						node->Child[ current_child ].node     = node->Child[ j ].node;
						node->Child[ current_child ].door     = node->Child[ j ].door;			
						node->Child[ current_child ].moveCost = node->Child[ j ].moveCost;

						memcpy( node->Child[ current_child ].maxheight, node->Child[ j ].maxheight, sizeof( node->Child[ current_child ].maxheight ) );
						}
			...
			node->numChildren = number_children_left;
			}
		}
	...
	Com_Printf( "\nOptimized path nodes\n" );
	Com_Printf( "  Kept %d paths\n", kept_paths );
	Com_Printf( "  Dropped %d paths\n", dropped_paths );
   }
