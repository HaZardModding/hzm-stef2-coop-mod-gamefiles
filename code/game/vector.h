//-----------------------------------------------------------------------------
//
//  $Logfile:: /red_code/game/vector.h                                       $
// $Revision:: 3                                                              $
//   $Author:: Steven                                                         $
//     $Date:: 12/08/00 1:21p                                                 $
//
...
//
// $Log:: /red_code/game/vector.h                                            $
// 
// 3     12/08/00 1:21p Steven
// Made most vector references const.
//
...
class Vector
	...
	public:
		...
		friend	Vector	operator+( const Vector &a, vec3_t b );
					...
					Vector&	operator+=( const char *text );
		friend	Vector	operator-( const Vector &a, const Vector &b );
		friend	Vector	operator-( vec3_t a, const Vector &b );
		friend	Vector	operator-( const Vector &a, vec3_t b );
					Vector&	operator-=( const Vector &a );
      ...
      friend	int		operator!=(	vec3_t a, const Vector &b );
		friend	int		operator!=(	const Vector &a, vec3_t b );
              	int		FuzzyEqual( const Vector &b, float epsilon );
              	int		FuzzyEqual( vec3_t b, float epsilon );
					Vector&	CrossProduct( const Vector &a, const Vector &b	);
					Vector&	CrossProduct( vec3_t a, const Vector &b	);
					Vector&	CrossProduct( const Vector &a, vec3_t b	);
					float		length( void );
...
inline Vector& Vector::operator+=
	...
	if ( text )
		{
		sscanf( text, "%f %f %f", &new_x, &new_y, &new_z );
