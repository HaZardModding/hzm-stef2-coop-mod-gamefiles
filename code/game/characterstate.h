//-----------------------------------------------------------------------------
//
//  $Logfile:: /red_code/game/characterstate.h                               $
// $Revision:: 8                                                              $
//     $Date:: 6/14/01 10:41a                                                 $
//
...
//
// $Log:: /red_code/game/characterstate.h                                    $
// 
// 8     6/14/01 10:41a Sketcher
// More work on splitting actor legs and torso
// 
// 7     4/02/01 2:11p Sketcher
...
// 
// 5     12/11/00 3:18p Steven
// Made str passing const references.
// 
// 4     12/11/00 11:56a Steven
// Start using const in some places.
// 
...
class State : public Class
   ...
   private :
      ...
      Container<str>             exitCommands;
		Container<str>					ignoreGlobalStates;
      ...
      void                       readStates( Script &script );
		void								readIgnoreGlobalStates( Script &script );
   ...
   public :
      ...
      void                       GetTorsoAnims( Container<const char *> *c );
		...
		void								addStateToIgnoreList( const str &StateToIgnore );
		qboolean							isStateInIgnoreList( const str &StateToCheck );
   };
...
inline void State::addStateToIgnoreList
	(
	const str &stateToIgnore
	)

	{
	ignoreGlobalStates.AddObject( stateToIgnore );
	}
...
class StateMap : public Class
   ...
   public :
      ...
      State                      *FindState( const char *name );
		const char						*Filename();
		void								ReadStates( const char *filename );
