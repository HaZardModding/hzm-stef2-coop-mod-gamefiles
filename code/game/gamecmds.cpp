//-----------------------------------------------------------------------------
//
//  $Logfile:: /red_code/game/gamecmds.cpp                                   $
// $Revision:: 4                                                              $
//     $Date:: 3/28/01 3:25p                                                  $
//
...
void G_Say
   ...
   if ( dedicated->integer )
		{
      gi.SendServerCommand( NULL, "hudprint \"%s\"", text );
		}
   ...
   if ( g_gametype->integer == GT_ARENA )
      ...
      if ( ent->entity )
         {
         dmManager.ArenaPrint( ent->entity, text );
         }
      }
   else
      {
   	for( j = 0; j < game.maxclients; j++ )
         ...
         gi.SendServerCommand( other-g_entities, "hudprint \"%s\"", text );
         }
		}
	}
