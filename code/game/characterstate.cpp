//-----------------------------------------------------------------------------
//
//  $Logfile:: /red_code/game/characterstate.cpp                             $
// $Revision:: 10                                                             $
//     $Date:: 6/14/01 10:41a                                                 $
//
...
//
// $Log:: /red_code/game/characterstate.cpp                                  $
// 
// 10    6/14/01 10:41a Sketcher
// More work on splitting actor legs and torso
// 
// 9     4/10/01 3:37p Steven
// Fixed a problem with entrycommands overrunning to the next line sometimes.
// 
// 8     4/02/01 2:11p Sketcher
...
// fully implemented
// 
// 7     3/28/01 3:24p Steven
// Added warning printfs.
// 
// 6     1/10/01 9:43a Sketcher
// Updated Enemy Management
// 
// 5     12/20/00 2:46p Sketcher
// Fixed a few newly discovered bugs with the new parameterized format for
// conditionals
// 
// 4     12/11/00 3:16p Steven
...
void State::readIgnoreGlobalStates
	(
	Script &script
	)
   ...
   if ( !script.TokenAvailable( true ) )
      {
      gi.Error( ERR_DROP, "%s: Expecting Ignore States on line %d.\n", script.Filename(), script.GetLineNumber() );
      }

   // Read in the ignore states arguments if there are any
...
qboolean State::isStateInIgnoreList
	(
	const str &StateToCheck
	)
		
	{
   str listState;

	for( int i = 1; i <= ignoreGlobalStates.NumObjects(); i++ )
      {
		listState = ignoreGlobalStates.ObjectAt(i);
		if( !Q_stricmp( listState.c_str(), StateToCheck.c_str() ) )
			return qtrue;
      }
...
const char *State::getHeadBehaviorName
   (
   void
   )
...
const char *State::getEyeBehaviorName
   (
	void
	)
...
const char *State::getTorsoBehaviorName
   (
	void
	)
...
StateMap::StateMap
   ...
   for(int i = 1; i <= stateList.NumObjects(); i++ )
      {
...
State *StateMap::GotoGlobalState
 (
 State *CurrentState, 
 State *newState
 )
 ...
 {
   if( CurrentState->isStateInIgnoreList( newState->getName() ) )
		{
		return CurrentState;
		}
