//-----------------------------------------------------------------------------
//
//  $Logfile:: /red_code/game/compiler.cpp                                    $
// $Revision:: 1                                                              $
//     $Date:: 1/16/01 2:13p                                                  $
//
...
//
// $Log:: /red_code/game/compiler.cpp                                         $
// 
// 1     1/16/01 2:13p Steven
// 
// 11    7/27/00 1:56p Pmack
// dose found a bug. fixed it.
// 
// 9     5/04/00 6:32p Jimdose
...
// 
// 5     3/31/00 3:48p Jimdose
// pre-initialized global are now differentiated from immediates so that
// changing their value doesn't change the value of immediates
// 
//
// DESCRIPTION:
// 
...
/*
      {"=", "STORE_FTOS", 6, true, &def_float, &def_string, &def_string},
*/
		// PGM -- dose found a bug in the above line. the below line is corrected.
...
============
*/
def_t *Compiler::ParseFunctionCall
   (
   def_t *func
   )
		...
		do
         ...
         switch( e->type->type )
            ...
            default:
               ...
               lex.ParseError( "No appropriate operator for storing type '%s'", e->type->def->name );
               break;
            }
...
============
*/
def_t *Compiler::ParseObjectCall
   (
   def_t *func,
   def_t *object
   )
...
============
*/
function_t Compiler::ParseImmediateStatements
   (
   type_t *type,
...
void Compiler::ParseFunctionDef
   (
	type_t *type,
	const char *name
   )
...
void Compiler::ParseVariableDef
   (
	type_t *type,
	const char *name
   )
	...
	while( lex.pr_token_type != tt_eof )
	   {
		try
         ...
         // outside all functions
		   ...
		   {
			if ( ++program.pr_error_count > MAX_ERRORS )
            {
				break;
            }
