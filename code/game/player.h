...
// * works
// * 
// * 25    3/15/01 9:04a Sketcher
// * Added Standard Armor
// 
// 11    2/07/01 1:23p Sketcher
// Changed the Heuristics System File Layout
// 
// 10    1.02.01 5:43p Ericf
// Fixed crouch firing
// 
// 9     1/31/01 9:35a Sketcher
// Working on the Heuristics System - Intermediate Check-in
// 
// 8     1/30/01 2:41p Sketcher
// The Heuristics system will now set a skill level value for the level
// 
// 7     30.01.01 9:47a Ericf
// Removed lots of code... rope, monkey bars, pipe climbs...
// 
// 6     26.01.01 1:24p Ericf
// Added weaponreload event
// 
// 5     1/23/01 10:37a Sketcher
// Added Code to allow for Saving and Loading of Heuristic File
// 
// 4     1/11/01 2:24p Sketcher
...
extern Event EV_Player_Spectator;
...
class Player : public Sentient
	...
	private:
      ...
      float             airspeed;
		...
		// Ghost Shirt Stuff
		float					GhostShirtValue;
		float             MaxGhostShirtValue;
      ...
      // Multiplayer
      int               num_deaths, num_kills, num_won_matches, num_lost_matches;
      qboolean          spectator;
      teamtype_t        dm_team;  // hope this isn't used!
      qboolean          allow_fighting;
      MultiplayerArena *_arena;
      Team             *_team;
      bool              voted;
      int               votecount;
	...
	public:
      ...
      CLASS_PROTOTYPE( Player );
		...
		void					InitGhostShirt( void );
		void              InitArmorValue( void );
      ...
      void					SetReverb( int type, float level );
		...
		void					SetGhostShirt( float shirt );		
		float					GetGhostShirt( void ) { return GhostShirtValue;		}
		void					AdjustGhostShirt( float adjustment );
		void					UpdateGhostShirtVar();
		
		void              SetArmorValue( int armorVal );
		int        			GetArmorValue( void ) { return ArmorValue;			}
		void              AdjustArmorValue( int adjustment );
      ...
      void              CameraCut( Camera *ent );
		...
		void		         SetPlayerView( Camera * camera, const Vector &position, float cameraoffset, const Vector &ang, const Vector &vel, float camerablend[ 4 ], float camerafov );
		void		         SetupView( void );
      ...
      Entity            *FindEnemyInFOVFromTagWithOffset( float fov, float maxdist, const str &tagname, const Vector &offset );
      void              DumpState( Event *ev );
      void              ForceTorsoState( Event *ev );

      void              SetHeadTarget( Event *ev );
      Vector            GetAngleToTarget( Entity *ent, const str &tag, float yawclamp, float pitchclamp, const Vector &baseangles );
      ...
      //void              SetWaterPower( Event *ev );
		void					SetGhostShirt( Event *ev );
		void					AdjustGhostShirt( Event *ev );
      ...
      void              SafeHolster( qboolean putaway );
		inline powerup_t	GetPowerupType() { return poweruptype; }
		inline int			GetFov() { return fov; }
      ...
      void              WeaponsNotHolstered( void );
		...
		qboolean				GetCrouch();
		qboolean				CheckLadder();
		void					TrainJump( Event *ev );
      // Deathmatch arena stuff
      void              Score( Event *ev );
      void              Join_DM_Team( Event *ev );
      void              Create_DM_Team( Event *ev );
      void              Leave_DM_Team( Event *ev );
      void              Join_DM_Arena( Event *ev );
      void              Leave_DM_Arena( Event *ev );
      void              RefreshArenaUI( void );
      void              RefreshArenaUI( Event *ev );
      teamtype_t        GetTeam( void );
      void              SetTeam( teamtype_t teamtype );
      qboolean          IsSpectator( void );
      void              BeginFight( void );
      void              EndFight( void );
      void              EnterArena( int health, int water );
      void              Spectator( Event *ev );
      void              Spectator( void );
      void              UpdateStatus( const char *txt );
      void              InitArenaSpawnPoint( void );

      inline MultiplayerArena * GetMultiplayerArena( void )                    { return _arena; };
      inline Team*              GetMultiplayerTeam(  void )                    { return _team; };
      inline void               SetMultiplayerArena( MultiplayerArena *arena ) { _arena = arena; };
      inline void               SetMultiplayerTeam( Team *team )               { _team  = team; };
      
	  void              WarpToPoint( Entity *ent );
      void              ArmorDamage( Event *ev );
      void              HUDPrint( const char *txt );
      void              Disconnect( void );
      inline int        GetNumKills( void ) { return num_kills; };
      inline int        GetNumDeaths( void ) { return num_deaths; };
      ...
      void              WonMatch( void );
      void              LostMatch( void );
      inline int        GetMatchesWon( void ) { return num_won_matches; };
      inline int        GetMatchesLost( void ) { return num_lost_matches; };
		...
		void					SetAtHitchingPost(qboolean val, Vector hangles, Vector horigin, qboolean honce);
		qboolean				GetAtHitchingPost();
...
inline void Player::Archive
   ...
   arc.ArchiveSafePointer( &fakePlayer );
	arc.ArchiveFloat( &GhostShirtValue );

	if ( arc.Loading() )
		UpdateGhostShirtVar();

	arc.ArchiveInteger( &ArmorValue );
