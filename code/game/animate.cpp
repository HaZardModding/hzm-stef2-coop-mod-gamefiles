//-----------------------------------------------------------------------------
//
//  $Logfile:: /red_code/game/animate.cpp                                    $
// $Revision:: 7                                                              $
//   $Author:: Sketcher                                                       $
//     $Date:: 8/07/01 8:52a                                                  $
//
...
//
// $Log:: /red_code/game/animate.cpp                                         $
// 
// 7     8/07/01 8:52a Sketcher
// More vehicle work
// 
// 6     4/05/01 5:48p Ericf
// Added a GetName function
// 
// 5     12/08/00 9:06a Steven
// Removed move_time.
// 
// 4     11/22/00 12:02p Steven
// Comments
//
...
void Animate::NewAnim
   ...
   totaltime = gi.Anim_Time( self->edict->s.modelindex, animnum );
   ...
   *frametime = gi.Frame_Time( self->edict->s.modelindex, animnum, 0 );
   ...
   has_commands = gi.Anim_HasCommands( self->edict->s.modelindex, animnum );
   ...
   if ( part == legs )
      ...
      if ( length > MINIMUM_DELTA_MOVEMENT )
         ...
         flags = gi.Anim_Flags( self->edict->s.modelindex, animnum );
   ...
   if (
      ...
      switch( part )
         ...
         case legs:
	         if ( animDoneEvent )
		         {
	            self->PostEvent( *animDoneEvent, 0 );
		         }
            break;
         case torso:
	         if ( torso_animDoneEvent )
		         {
	            self->PostEvent( *torso_animDoneEvent, 0 );
		         }
            break;
         default:
            break;
