//-----------------------------------------------------------------------------
//
//  $Logfile:: /red_code/game/level.cpp                                      $
// $Revision:: 10                                                             $
//     $Date:: 5/03/01 4:37p                                                  $
//
...
//
// $Log:: /red_code/game/level.cpp                                           $
// 
// 10    5/03/01 4:37p Steven
// Worked on water, slime, & lava screen blend stuff.
// 
// 9     4/24/01 7:33a Steven
// Added dialog text.
// 
// 8     4/03/01 2:06p Steven
// Added a warning if the world is not spawned correctly.
// 
// 7     4/03/01 10:18a Steven
// Made it check to see if a precache function was available before trying to
// run it.
// 
// 5     1/30/01 2:41p Sketcher
// The Heuristics system will now set a skill level value for the level
// 
// 4     1/23/01 8:55a Steven
// Added level vars and game vars back in and added some level precaching
// stuff back in.
// 
// 3     1/16/01 2:11p Steven
// Script C merge.
// 
// 2     12/08/00 9:07a Steven
// Moved bind stuff to a pluggable module.
//
// 43    10/02/00 11:55a Aldie
// Added voting
//
// 42    9/28/00 6:51p Aldie
// Added more multiplayer options (voting)
//
// 41    9/20/00 6:58p Aldie
// Another temporary checkin for deathmatch stuff
//
// 40    9/18/00 6:00p Aldie
// Periodic update for FakkArena deathmatch code.
//
// 39    9/12/00 7:03p Aldie
// Complete overhaul of the deathmatch stuff I wrote 2 weeks ago.  More to
// come.  Intermediate checkin
//
// 38    9/01/00 7:09p Aldie
      ...
      program.Load( game_script );
		...
		if ( gamescript )
			...
			if ( gamescript->labelExists( "precache" ) )
				...
				if ( precache_script )
					precache_script->DelayedStart( 0 );
				}
...
void Level::NewMap
   ...
   current_entities = entities;
	...
	dialogText.LoadEntries( "standard" );
	dialogText.LoadEntries( mapname );
...
void Level::Precache
   ...
   //
	...
	if ( game_script.length() )
      ...
      COM_StripExtension( game_script.c_str(), stripped );
      Com_sprintf( precache_script, sizeof( precache_script ), "%s_precache.scr", stripped );
		if ( gi.FS_ReadFile( precache_script, NULL, qtrue ) != -1 )
				level.consoleThread->Parse( precache_script );
