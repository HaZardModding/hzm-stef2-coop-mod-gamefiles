//-----------------------------------------------------------------------------
//
//  $Logfile:: /red_code/game/mover.cpp                                      $
// $Revision:: 5                                                              $
//   $Author:: Steven                                                         $
//     $Date:: 12/11/00 9:33a                                                 $
//
...
void Mover::MoveDone
   ...
   Vector move;
	...
	if ( moveflags & MOVE_ORIGIN )
		...
		move = finaldest - self->localorigin;
		}
	else
		{
		move = vec_zero;
	...
	if ( moveflags & MOVE_ANGLES )
		...
		if ( ( self->localangles.x >= 360 ) || ( self->localangles.x < 0 ) )
			{
			self->localangles.x -= ( (int)self->localangles.x / 360 ) * 360;
			}
		if ( ( self->localangles.y >= 360 ) || ( self->localangles.y < 0 ) )
			{
			self->localangles.y -= ( (int)self->localangles.y / 360 ) * 360;
			}
		if ( ( self->localangles.z >= 360 ) || ( self->localangles.z < 0 ) )
			{
			self->localangles.z -= ( (int)self->localangles.z / 360 ) * 360;
			}
		}
...
void Mover::MoveTo
   ...
   if ( !moveflags )
	...
	vdestdelta = tdest - self->localorigin;
	angdestdelta = angdest - self->localangles;

	if ( tdest == self->localorigin )
		{
...
//	traveltime = ( float )( (int)traveltime ) * FRAMETIME;
	...
	if ( moveflags & MOVE_ANGLES )
		{
		self->avelocity = angdestdelta * ( 1 / traveltime );
		}
