//-----------------------------------------------------------------------------
//
//  $Logfile:: /red_code/game/vehicle.cpp                                    $
// $Revision:: 18                                                             $
//   $Author:: Sketcher                                                       $
//     $Date:: 8/17/01 2:04p                                                  $
//
...
//
// $Log:: /red_code/game/vehicle.cpp                                         $
// 
// 18    8/17/01 2:04p Sketcher
// Fixed some errors with Vehicle and traceattack
// 
// 17    8/08/01 9:34a Sketcher
// Added an event to make the player use a specified entity, and fixed getting
// on and off the stupid horse
// 
// 16    8/07/01 11:40a Sketcher
// Took Z-value out of the equation when calculating distance to waypointnodes
// 
// 15    8/07/01 10:19a Sketcher
// Fixed, (hopefully) a possible null pointer problem
// 
// 14    8/07/01 8:52a Sketcher
// More vehicle work
// 
// 13    8/02/01 2:51p Sketcher
// More Vehicle Prediction Work... Oh how I hate it.
// 
// 12    8/02/01 12:03p Sketcher
// Trying to get vehicles to work better.  Still need to fix prediction
// 
// 11    7/27/01 5:45p Sketcher
// Switched from Enum to a strategy pattern for movement handling
// 
// 10    7/27/01 8:34a Sketcher
// Fixed a warning
// 
...
void Vehicle::Postthink
   ...
   if ( drivable )
      ...
      for ( index = 0; index < 4; index++ )
         ...
         if ( trace.fraction > 0 && trace.fraction != 1.0f && !trace.startsolid )
            {
...
CLASS_DECLARATION( Vehicle, HorseVehicle, "script_horsevehicle" )
	{
		{ &EV_HorseVehicle_SetSpeed ,    SetSpeed	},
		{ &EV_HorseVehicle_SetMoveMode , SetVehicleMoveMode		},
		{ &EV_HorseVehicle_SetForcedForward, SetForcedForwardSpeed		},
		{ &EV_Vehicle_AnimDone,                      AnimDone	},
		{ &EV_Driver_AnimDone,                      DriverAnimDone	},


		//Move Mode Events
		{ &EV_FollowPath_SetWayPointName,  PassToMoveMode		},
		{ NULL, NULL }
	};
...
qboolean HorseVehicle::Drive(	usercmd_t *ucmd )
   ...
   //driver->client->ps.pm_flags |= PMF_NO_PREDICTION;
	...
	driver->client->ps.vehicleoffset[2] = origin[2] + 160;	
...
void HorseVehicle::_CalculateOrientation( )
	{
...
void HorseVehicle::_InitializeJump()
	{
	_jumptime = level.time + .25;
	_holdtime = _jumptime + .15;
...
CLASS_DECLARATION( VehicleMoveMode, HVMoveMode_FollowPath, "FollowPath_MoveModeStrategy" )
	{
		{ &EV_FollowPath_SetWayPointName,  SetWaypointName },
		{ NULL, NULL }
	};
